...
RSpec.describe 'Legion::LLM.ask' do
  ...
  describe 'daemon path: :immediate' do
    ...
    it 'does not call chat_direct' do
      expect(Legion::LLM::Inference).not_to receive(:chat_direct_raw)
  ...
  describe 'daemon path: :unavailable falls through to direct' do
    ...
    before do
      ...
      allow(Legion::LLM::Inference).to receive(:chat_direct_raw).and_return(session)
    end

    it 'falls through and calls chat_direct' do
      expect(Legion::LLM::Inference).to receive(:chat_direct_raw)
      Legion::LLM.ask(message: 'hello')
    end
  ...
  describe 'direct path passes model and provider through' do
    ...
    it 'forwards model to chat_direct' do
      expect(Legion::LLM::Inference).to receive(:chat_direct_raw).with(hash_including(model: 'gpt-4o')).and_return(session)
      Legion::LLM.ask(message: 'hello', model: 'gpt-4o')
    end

    it 'forwards provider to chat_direct' do
      expect(Legion::LLM::Inference).to receive(:chat_direct_raw).with(hash_including(provider: :openai)).and_return(session)
      Legion::LLM.ask(message: 'hello', provider: :openai)
    end

    it 'forwards tier to chat_direct' do
      expect(Legion::LLM::Inference).to receive(:chat_direct_raw).with(hash_including(tier: :cloud)).and_return(session)
      Legion::LLM.ask(message: 'hello', tier: :cloud)
    end

    it 'forwards intent to chat_direct' do
      expect(Legion::LLM::Inference).to receive(:chat_direct_raw).with(hash_including(intent: { privacy: :strict })).and_return(session)
