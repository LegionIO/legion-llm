...
RSpec.describe 'legion-llm load phase' do
  it 'loads shared settings when requiring legion/llm directly' do
    code = <<~RUBY
      ...
      raise 'llm defaults missing' unless Legion::LLM::Settings.value(:enabled) == true
      Legion::Settings[:llm][:default_model] = 'override-model'
      raise 'override missing' unless Legion::LLM::Settings.value(:default_model) == 'override-model'
    RUBY
