...
RSpec.describe 'Router discovery integration' do
  ...
  describe 'when model is not available' do
    before do
      ...
      allow(Legion::LLM::Inventory::Discovery).to receive(:model_available?).with('llama3.1:8b', provider: :ollama, instance: nil).and_return(false)
  ...
  describe 'when model is available and fits in memory' do
    before do
      configure_routing(rules: rules_with_local)
      allow(Legion::LLM::Inventory::Discovery).to receive(:model_available?).with('llama3.1:8b', provider: :ollama, instance: nil).and_return(true)
      allow(Legion::LLM::Inventory::Discovery).to receive(:model_size).with('llama3.1:8b', provider: :ollama, instance: nil).and_return(4_700_000_000)
      allow(Legion::LLM::Discovery::System).to receive(:available_memory_mb).and_return(32_000)
    end
  ...
  describe 'non-local rules are unaffected' do
    ...
    it 'does not check discovery for cloud rules' do
      expect(Legion::LLM::Inventory::Discovery).not_to receive(:model_available?)
