...
RSpec.describe Legion::LLM::Discovery::MemoryGate do
  ...
  describe '.allow?' do
    ...
    context 'with local provider ollama' do
      ...
      it 'returns true when model fits in memory' do
        ...
        allow(Legion::LLM::Discovery).to receive(:model_size).with('llama3:latest', provider: :ollama, instance: nil).and_return(4_000_000_000)
      ...
      it 'returns false when model exceeds available memory' do
        ...
        allow(Legion::LLM::Discovery).to receive(:model_size).with('llama3:70b', provider: :ollama, instance: nil).and_return(12_000_000_000)
  ...
  describe '.estimated_model_mb' do
    ...
    it 'returns 4096 default when model size is zero' do
      allow(Legion::LLM::Discovery).to receive(:model_size).and_return(0)
