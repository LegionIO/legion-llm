...
RSpec.describe Legion::LLM::Call::SelectionDispatch, :ssot_v3 do
  ...
  it 'rehydrates hash messages for every exact message operation without mutating the input' do
    ...
    canonical_message = Legion::Extensions::Llm::Canonical::Message.build(role: :assistant, content: 'canonical')
    raw_messages = [
    ...
    contexts.each do |operation, context|
      stream_block = proc { |_chunk| } if operation == :stream_chat
    ...
    calls.each do |call|
      ...
      # A canonical message passes through by identity — no re-normalization.
      expect(call[:messages].last).to equal(canonical_message)
      ...
      # 0.8.0 law (kit T4): cache_control IS a Message member and survives
      # build/to_h round-trips, including the fleet wire.
      expect(call[:messages][1].to_h).to include(cache_control: { type: 'ephemeral' })
    end
    expect(calls.find { |call| call[:operation] == :stream_chat }[:block]).to be_a(Proc)
    expect(raw_messages[0, 3]).to eq(original)
  end

  it 'rejects a non-canonical message object type before acquiring a callable lease' do
    ctx = chat_context
    handle = ctx.selection.callable_handle

    expect do
      described.call(attempt_context: ctx, arguments: { messages: [Object.new] })
    end.to raise_error(ArgumentError, /must be Canonical::Message, String, or Hash/)
    expect(handle.reference_count).to eq(0)
  end
