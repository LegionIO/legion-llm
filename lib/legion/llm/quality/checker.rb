      ...
      module Checker
        ...
        class << self
          ...
          def effective_content(response)
            content = response.respond_to?(:text) ? response.text.to_s :
              (response.respond_to?(:content) ? response.content.to_s : (response.respond_to?(:[]) && response[:result] && response[:result].to_s))
