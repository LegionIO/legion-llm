  ...
  module LLM
    module Scheduling
      ...
      class << self
        ...
        def enabled?
          scheduling_config.fetch(:enabled, false) == true
        end
        ...
        def next_off_peak(time = Time.now.utc)
          ...
          max_defer = scheduling_config.fetch(:max_defer_hours, 8)
        ...
        def status
          {
            ...
            max_defer_hours: scheduling_config.fetch(:max_defer_hours, 8)
          }
        end
        ...
        def scheduling_config
          s = Legion::Settings[:llm]&.dig(:scheduling) || Legion::Settings[:llm]&.dig('scheduling') || {}
          s.is_a?(Hash) ? s.transform_keys(&:to_sym) : {}
        rescue StandardError => e
          handle_exception(e, level: :warn, operation: 'llm.scheduling.config')
          {}
        end

        def peak_range
          raw = scheduling_config[:peak_hours_utc]
        ...
        def defer_intents
          raw = scheduling_config[:defer_intents]
